...
CarImuNode::CarImuNode() : Node("carImuNode") {
    ...
    RCLCPP_INFO(rclcpp::get_logger("CarImuNode"), "小车惯导模块节点初始化完成");
}

CarImuNode::~CarImuNode() {
    delete serial;
    RCLCPP_INFO(rclcpp::get_logger("CarImuNode"), "小车惯导模块节点销毁");
}

int CarImuNode::work(TasksManager& tm, Task& task) {
    serial = new serial::Serial(task.deviceInfo.node, task.deviceInfo.baudRate);
    if (!serial->isOpen()) serial->open();
    int microseconds = 1000000 / this->frequency;
    RCLCPP_INFO(rclcpp::get_logger("CarImuNode"), "小车惯导模块节点开始运行");
    while (task.running) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("CarImuNode"), "小车惯导模块节点终止运行");
    return 0;
}

void CarImuNode::rawDataHandler(std::vector<uint8_t> arr, int count) {
    rawDataBuffer.insert(rawDataBuffer.end(), arr.begin(), arr.end());
    if (rawDataBuffer.size() < 11) return;
    // 找到开头节点
    int i = 0;
    while ((i + 1) < rawDataBuffer.size() && !((rawDataBuffer[i] == 0x55) && (rawDataBuffer[i + 1] == 0x52))) i++;
    // 删除前面的无效段
    ...
    while (i < rawDataBuffer.size() && rawDataBuffer.size() - i >= 11) {
        if (rawDataBuffer[i] == 0x55) {
            ...
            switch (rawDataBuffer[i + 1]) {
                // case 0x50: {
                //     //时间
                //     // dataFrame.timestamp = (unsigned short)(((unsigned short)rawDataBuffer[9] << 8) | rawDataBuffer[8]) +
                //     //                       (((rawDataBuffer[i + 4] /*日*/ * 24 + rawDataBuffer[i + 5] /*时*/) * 60 + rawDataBuffer[i + 6] /*分*/) * 60 +
                //     //                        rawDataBuffer[i + 7] /*秒*/) *
                //     //                           1000;
                //     dataFrame.timestamp = this->now().nanoseconds();
                //     // printf("time :%u\n", timestamp);
                //     break;
                // }
                // case 0x51: {
                ...
                case 0x52: {
                    ...
                    // message::msg::ImuAngularVelocity* av = &dataFrame.angular_velocity;
                    // printf("角速度 x:%lf y:%lf z:%lf\n", (*av).angular_velocity_x, (*av).angular_velocity_y, (*av).angular_velocity_z);
...
void CarImuNode::publish(message::msg::ImuData::SharedPtr& imuData) {
    if (!publisher) {
        // publisher被销毁
        return;
    }
